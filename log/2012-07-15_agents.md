...
Right, so draft 2. Thinking big picture.

### Definitions:

 - "agent": an independent client which has been allocated in the document
 - "state": markup, styling, and javascript which populates an agent
 - "server": javascript or remote host which serves resources
 - "program": a combination of states and/or resources which form a complete tool or application

### Needs:

 - programs need to be able to alter behavior at runtime, not just at init
 - programs need to run their own initialization processes with minimal intervention and configuration
 - programs need standard operating mechanisms that can interface with the shell (via requests)
 - resources and states need to unload from memory when no longer in use
 - servers need to discover each other and cooperate when possible
 - URIs would ideally be conflict-free, consistent, and human-usable; may need to relax one of those 3

### Available tools:

 - link registries
 - dynamic configuration resources (eg a queryable index)
 - dynamic response handling in the agent according to the active state
 - resource interfaces to active programs

### Processes:

System

 - 1. ON credential submit (from initial login state)
   - a. create session with user config service
   - b. get user env config
   - c. load startup programs into subagents
 - 2. ON program load
   - a. populate agent with program-loader state
   - b. loader state requests config
   - c. ...creates program servers
   - d  ...(optionally) creates sub-agents and populates them with states
   - e. ...populates given agent with program-main state
 - 3. ON program shutdown
   - a. populate agent with program-shutdown state
   - b. shutdown state issues delete requests to program servers
   - c. shutdown state goes dormant
   - d. (optionally) destroy agent
 - 4. ON request
   - a. owning agent request handlers are fired
   - b. owning agent response handlers are added to response promise
   - c. request is dispatched
   - d. response handlers interpret request

Agent defaults

 - 5. ON response (Content-Type: application/html+json)
   - a. link registry is set to response link header
   - if load function is present:
     - b. load function is called with response as a param
   - else:
     - c. response HTML is written to document
   - d. render function (if present) is called
 - 6. ON response (Content-Type: *)
   - a. link registry is set to response link header
   - b. response is converted to string and written to document

### Notes

Predefined state changes (eg 2a, 3a) occur by following links which are registered to the Agent.

All agent behavior is according the the active state. Default behaviors are designed to enable state-changes.

There is no prescription for when a state change ends the life-cycle of a program; rather, handlers (and response CBs) decide how to alter the agent. This process may need to adjust as the real use-cases become more clear.

Agents are created and managed by parent agents; the top-most agent 
...
```
todo
--tool to require() scripts
--tool to add servers
--tool to remove servers
```

---

Hmm. Looking at all this and thinking, man, that's a lot of process. I'm not sure the system needs it yet. Interpretting responses by agent state is an interesting idea; let's leave it at that for now.
