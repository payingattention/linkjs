...
### Processes:
 ...
 - 1. ON credential submit (from blank slate)
   - a. login with user config service
   - b. get user env config
   - c. load startup programs
 - 2. ON program load
 ...
 - 4. ON state populate
   - a. html (if present in response) fills agent DOM element
   - b. onrender callback (if present) runs
     - registers request event listeners to handle responses
     - registers DOM event liseners & runs UI code
