...
window.env = {
    ...
    , exec: function(name) {
        ...
        $.get('/proxy' + name, '', function(data, textStatus, jqXHR) {
            // evaluate the function definiton and bind to script_func
            var script_func = new Function(data)
            // call & pass remaining arguments
            script_func.apply(null, other_args)
        }, 'text')
    }
}