define(["./request", "./response"], function(Request, Response) {
    ...
    var remoteDispatcherFunction = function(request, opt_cb) {
    ...
    var findModules = function(uriRegexp, opt_keyBuilder) {
        ...
        for (var i=0; i < appModules.length; i++) {
            ...
            var match = uriRegexp.exec(module.uri());
            if (match) {
                // Generate the key & store
                var key = (opt_keyBuilder ? opt_keyBuilder(match) : i);
                matchedModules[key] = { uri:module.uri() };
            }
        }
    ...
    var findHandlers = function(request) {
        ...
        for (var i=0; i < appModules.length; i++) {
            ...
            if (relUriIndex != -1) {
                ...
                for (var j=0; j < module.handlers.length; j++) {
                    var handler = module.handlers[j];
                    var urimatch;
                    // Test URI first
    ...
    // Updates/gets remote dispatcher function
    var remoteDispatcher = function(opt_cb) {
        if (opt_cb) {
            remoteDispatcherFunction = opt_cb;
        }
        return remoteDispatcherFunction;
    }
    ...
    var useJQuery = function($) {
        remoteDispatcher(function(request, opt_cb) {
            $.ajax({
                ...
                opt_cb && opt_cb(new Response(xhr.status, xhr.statusText, xhr.getAllResponseHeaders(), xhr.responseText));
            });
        });
    };
