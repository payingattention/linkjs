define(['require', './response', './util'], function(require, Response, Util) {
    ...
    Request.prototype.nextHandler = function(response) {
        ...
        if (!handler) {
            ...
            if (!response) { console.log(this, response); response = new Response(404, 'Not Found'); }
            // Log
            var linkApp = require('./app');
            if (linkApp.logMode('traffic')) {
    ...
    Request.Factory = function(opt_method, opt_uri, opt_headers, opt_body, opt_contenttype) {
        if (!opt_uri) { opt_uri = ''; }
        return function(opt_callUri, opt_callBody, opt_callContenttype) {
            // Build URI
            var uri = '';
            if (!opt_uri) { uri = opt_callUri; }
            else {
                // Given an array? Build uri from parts, insert opt_callUri at any falses
                if (Array.isArray(opt_uri)) {
                    uri = Util.arrayToString(opt_uri, opt_callUri);
                } else {
                    // Not array, just try to get a string out of it
                    uri = opt_uri.toString();
                    if (opt_callUri) { uri += opt_callUri; }
                }
            }

            // Settle on body
            var body = ((opt_callBody !== undefined) ? opt_callBody : opt_body);
            var contenttype = ((opt_callContenttype !== undefined) ? opt_callContenttype : opt_contenttype);

            // Make request
            return new Request(opt_method, uri, opt_headers, body, contenttype);
        }
    };
