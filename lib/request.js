define(['require', './response', './util'], function(require, Response, Util) {
    var midCounter = 0;
    // Request
    // =======
    ...
    var Request = function(opt_method, opt_uri, opt_headers, opt_body, opt_contenttype) {
        this.__headers = {};
        opt_method !== undefined      && this.method(opt_method);
        opt_uri !== undefined         && this.uri(opt_uri);
    ...
    Request.prototype.uri = function(opt_uri) {
        if (opt_uri !== undefined) { this.__uri = opt_uri; return this; }
        return this.__uri;
    };
    // method('post') -> this; method() -> 'post'
    Request.prototype.method = function(opt_method) {
        if (opt_method !== undefined) { this.__method = opt_method; return this; }
        return this.__method;
    };
    // header({ 'accept':'text/html' }) -> this; header('accept') -> 'text/html'
    Request.prototype.header = function(key_or_headerKVs, opt_value) {
    ...
    Request.prototype.body = function(opt_body, opt_contenttype) {
        if (opt_body === undefined) { return this.__body; }
    ...
    // (ASYNC) Runs dispatch and uses the response for the given request
    Request.prototype.pipeTo = function(destRequest) {
        this.dispatch(function(request, response) {
            destRequest.respond(response);
        });
    };
    ...
    Request.prototype.nextHandler = function(response) {
        // use cached response, in case a handler calls without regard to past responses
        if (!response) { response = this.__cur_response; }
        this.__cur_response = response;
        // find next handler
        ...
        if (!handler) {
            ...
            if (!handler.cb) { return; } // dispatch handler was optional
        }
    ...
    // Request generator object
    Request.Link = function(uri, headers) {
        this.uri = uri;
        this.headers = headers;
    };
    Request.Link.prototype.makeRequest = function(method, opt_body, opt_contenttype) {
        return new Request(method, this.uri, this.headers, opt_body, opt_contenttype);
    };
    Request.Link.prototype.follow = function(method, body, contenttype, cb, opt_context) {
        var request = this.makeRequest(method, body, contenttype);
        request.dispatch(cb, opt_context);
    };
    Request.Link.prototype.get = function(cb, opt_context) {
        this.follow('get', undefined, undefined, cb, opt_context);
    };
    Request.Link.prototype.post = function(body, contenttype, cb, opt_context) {
        this.follow('post', body, contenttype, cb, opt_context);
    };
