define(['require', './response'], function(require, Response) {
    ...
    Request.prototype.dispatch = function(opt_cb, opt_context) {
        ...
        if (linkApp.logMode('traffic')) {
            console.log('[traffic] >>', this.mid, this.__uri, '['+this.__headers['accept']+']', this);
        }
        ...
        if (this.__uri.charAt(0) != '#') {
            ...
            return linkApp.remoteDispatcher()(this, function(response) {
                if (linkApp.logMode('traffic')) {
                    console.log('[traffic] <<', self.mid, self.__uri, '['+response.header('content-type')+']', response);
                }
                opt_cb.call(opt_context, self, response);
            });
        }
    ...
    Request.prototype.nextHandler = function(response) {
        ...
        if (!handler) {
            ...
            var linkApp = require('./app');
            if (linkApp.logMode('traffic')) {
                console.log('[traffic] <<', this.mid, this.__uri, '['+response.header('content-type')+']', response);
            }
        }
