define(['link/module', 'link/request'], function(Module, Request) {
    // CLI
    // ===
    ...
    CLI.get({ uri:'^/tests/?$', 'content-type':'text/html' }, 'testsHandler');
    ...
    CLI.prototype.testsHandler = function(request) {
        ...
        var cmd = this.parse('a');
        var cmd = this.parse('a/b/c');
        var cmd = this.parse('#a/b/c');
        var cmd = this.parse('http://google.com');
        var cmd = this.parse('post a/b/c');
        var cmd = this.parse('post #a/b/c');
        var cmd = this.parse('post http://google.com');
        var cmd = this.parse('put a/b/c --pragma=test');
        var cmd = this.parse('put a/b/c --pragma="test"');
        var cmd = this.parse('put a/b/c --pragma=test --key=value');
        var cmd = this.parse('a [b]');
        var cmd = this.parse('a [b/c]');
        var cmd = this.parse('a [ b/c ]');
        var cmd = this.parse('a [ b/c ] d');
        var cmd = this.parse('a/b [ c/d ] e/f');
        var cmd = this.parse('a/b [ c/d ] e/f [g/h] i/j');
        var cmd = this.parse('get a/b [ c/d ] put e/f');
        var cmd = this.parse('get a/b --c=d --e="f" [ g/h ] put i/j --k=l');
        // Test exceptions for invalid inputs
        // :TODO:
        
        request.respond(200, 'done', 'text/html');
    };
