define(['link/module', 'link/app', 'link/request', 'link/response'], function(Module, linkApp, Request, Response) {
    ...
    CLI.prototype.testsHandler = function(orgRequest) {
        ...
        (new Request('post', '#/test/suite', {}, 'Parser')).dispatch(function(request, response) {
            ...
            var test = Request.Factory('post', suiteURI, { 'content-type':'jso/array' });
            var tests = [];
            ...
            // Build test suite
            testParse('a', [ { uri:'a', method:'get' } ]);
            testParse('a/b/c', [ { uri:'a/b/c', method:'get' } ]);
            testParse('#a/b/c', [ { uri:'#a/b/c', method:'get' } ]);
            testParse('http://google.com', [ { uri:'http://google.com', method:'get' }]);
            testParse('post a/b/c', [ { uri:'a/b/c', method:'post' } ]);
            testParse('post #a/b/c', [ { uri:'#a/b/c', method:'post' } ]);
            testParse('post http://google.com', [ { uri:'http://google.com', method:'post' } ]);
            testParse('put a/b/c --pragma=test', [ { uri:'a/b/c', method:'put', pragma:'test' } ]);
            testParse('put a/b/c --pragma="test"', [ { uri:'a/b/c', method:'put', pragma:'test' } ]);
            testParse('put a/b/c --pragma=test --key=value', [ { uri:'a/b/c', method:'put', pragma:'test', key:'value' } ]);
            testParse('a [b]', [ { uri:'a', method:'get', accept:'b' } ]);
            testParse('a [b/c]', [ { uri:'a', method:'get', accept:'b/c' } ]);
            testParse('a [ b/c ]', [ { uri:'a', method:'get', accept:'b/c' } ]);
            testParse('a [ b/c ] d', [
