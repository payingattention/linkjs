...
define(['link/module', 'link/app', 'link/request', 'link/response'], function(Module, linkApp, Request, Response) {
    ...
    var CLI = Module({}, function() {
        this.history = [];
        // Base resources
        this.addResource('/', this.cliResource);
        // Command resources
        this.addResource('/history', this.historyResource);
        this.addResource('/echo', this.echoResource);
        this.addResource('/test', this.testResource);
    });
    ...
    // Base resources
    // ==============
    CLI.prototype.cliResource = function(request) {
        ...
        } else if (request.matches({ method:'post', 'content-type':'text/plain' })) {
            // Run command
        ...
        var respond = function() {
            ...
            if (request.matches({ accept:'text/html' })) {
                ...
                var output = self.history[histId-1].output = [
                    ...
                    '<strong>',cmd,'</strong>',
                    '</p>',
                    '<p>',(finalResponse.ok() ? '&check; ' : '&times; ' ), msg,'</p>'
                ].join('');
                // Respond
                request.respond(200, output, 'text/html');
            }
        };
    ...
    CLI.prototype.historyHandler = function(request, response, urimatch) {
        ...
        if (request.header('accept') && !resp.matches({ 'content-type':request.header('accept') })) {
            return request.respond(new Response(415, 'media type mismatch (' + resp.header('content-type') + ' &rarr; ' + request.header('accept') + ')'));
        }
    ...
    // Commands
    // ========
    CLI.prototype.historyResource = function(request) {
        if (request.matches({ method:'get', accept:'text/html' })) {
            var html = '';
    ...
    CLI.prototype.echoResource = function(request) {
        if (request.matches({ method:'post' }) && request.body()) {
    ...
    CLI.prototype.testsResource = function(orgRequest) {
        if (!orgRequest.matches({ method:'get', accept:'text/html' })) {
            // :TODO: error responses
        }
