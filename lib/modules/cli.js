define(['link/module', 'link/app', 'link/request'], function(Module, linkApp, Request) {
    // CLI
    // ===
    ...
    CLI.post({ uri:'^/test/parses$' },                       'testParsesHandler');
    ...
    // Testing
    // =======
    CLI.prototype.testsHandler = function(request) {        
        // :TODO: log query param
        //this.parser.logging = true;

        this.testRequests = [];
        this.testThat(true, 'isok');
        this.testThat(false, 'fails');
        this.testThat(false, 'isok');
        this.testThat(5, 'equals', 5);
        this.testThat(true, 'equals', false);

        // Test parsing of valid inputs
        this.testThat('a', 'parses', [ { uri:'a', method:'get' } ]);
        this.testThat('a/b/c', 'parses', [ { uri:'a/b/c', method:'get' } ]);
        this.testThat('#a/b/c', 'parses', [ { uri:'#a/b/c', method:'get' } ]);
        this.testThat('http://google.com', 'parses', [ { uri:'http://google.com', method:'get' }]);
        this.testThat('post a/b/c', 'parses', [ { uri:'a/b/c', method:'post' } ]);
        this.testThat('post #a/b/c', 'parses', [ { uri:'#a/b/c', method:'post' } ]);
        this.testThat('post http://google.com', 'parses', [ { uri:'http://google.com', method:'post' } ]);
        this.testThat('put a/b/c --pragma=test', 'parses', [ { uri:'a/b/c', method:'put', pragma:'test' } ]);
        this.testThat('put a/b/c --pragma="test"', 'parses', [ { uri:'a/b/c', method:'put', pragma:'test' } ]);
        this.testThat('put a/b/c --pragma=test --key=value', 'parses', [ { uri:'a/b/c', method:'put', pragma:'test', key:'value' } ]);
        this.testThat('a [b]', 'parses', [ { uri:'a', method:'get', accept:'b' } ]);
        this.testThat('a [b/c]', 'parses', [ { uri:'a', method:'get', accept:'b/c' } ]);
        this.testThat('a [ b/c ]', 'parses', [ { uri:'a', method:'get', accept:'b/c' } ]);
        this.testThat('a [ b/c ] d', 'parses', [ { uri:'a', method:'get', accept:'b/c' }, { uri:'d', method:'post', 'content-type':'b/c' } ]);
        this.testThat('a/b [ c/d ] e/f', 'parses', [ { uri:'a/b', method:'get', accept:'c/d' }, { uri:'e/f', method:'post', 'content-type':'c/d' } ]);
        this.testThat('a/b [ c/d ] e/f [g/h] i/j', 'parses', [ { uri:'a/b', method:'get', accept:'c/d' }, { uri:'e/f', method:'post', 'content-type':'c/d', 'accept':'g/h' }, { uri:'i/j', method:'post', 'content-type':'g/h' } ]);
        this.testThat('get a/b [ c/d ] put e/f', 'parses', [ { uri:'a/b', method:'get', accept:'c/d' }, { uri:'e/f', method:'put', 'content-type':'c/d' } ]);
        this.testThat('get a/b --c=d --e="f" [ g/h ] put i/j --k=l', 'parses', [ { uri:'a/b', method:'get', accept:'g/h', c:'d', e:'f' }, { uri:'i/j', method:'put', 'content-type':'g/h', k:'l'  } ]);
        ...
        Request.batchDispatch(
            ...
            function() { // after all responses
                request.respond(200, resultHtml, 'text/html');
            }, 
            this // context
        );
    };
    // Test helper
    CLI.prototype.testThat = function() {
        var test = Array.prototype.splice.call(arguments, 1, 1);
        this.testRequests.push(new Request('post', this.uri() + '/test/' + test, null, arguments));
    };
    // Custom testing route
    CLI.prototype.testParsesHandler = function(request) {
        var params = request.body();
        var command = params[0], match = params[1];
        var cmdRequests = this.parse(command);
        if (cmdRequests.length != match.length) { return request.respond(500, command + " -- expected " + match.length + " cmdRequests (actual: " + cmdRequests.length + ")"); }
        for (var i=0; i < cmdRequests.length; i++) {
            if (match[i].uri && match[i].uri != cmdRequests[i].uri()) {
                return request.respond(500, command + " -- mismatch on request " + i + " uri - expected '" + match[i].uri + "', got '" + cmdRequests[i].uri() + "'");
            }
            if (!cmdRequests[i].matches(match[i])) {
                console.log(command, "expected", match[i], "received", cmdRequests[i]);
                return request.respond(500, command + " -- mismatch on request " + i + " - check log for details");
            }
        }
        request.respond(200, command);
    };
    ...
    CLI.prototype.parser.readHeaderSwitch = function() {
        ...
        /*match = this.reToken.exec(this.buffer.substring(i));
        if (!match) { return false; }
        var headerKey = match[1];
        i += match[0].length;*/
